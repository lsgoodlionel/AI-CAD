...
## 当前版本：v0.5.0-dev（feat/batch-review-and-model-base）

当前最新开发线为 `feat/batch-review-and-model-base`，已同步到 GitHub 同名分支；截至 2026-07-08，最新提交为 `9c4489e`。该分支在 `main` / `v0.4.0` 基础上继续推进**批量读图 / 整套审图（Phase 5）**、**工程 3D 模型基座（Phase 6）** 和 **3D 模型高精度重建（Phase 7）**，并完成 2309 张真实图纸数据质量修复。
