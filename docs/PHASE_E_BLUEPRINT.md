...
**收尾方案(蓝图末项,最后处理)**:
1. 仅对**结构/建筑平面图**跑高 DPI OCR(剖面/详图/机电按需),缩减图量;
2. 分块并行 + 降 DPI 自适应(大图先粗后精);
3. 可选 paddle GPU(需 GPU 环境);
4. 抽取任务拆「快速几何/变换(秒级)」与「慢速 OCR(异步低优先)」两级,
   让变换/矢量文字先铺满(A2 半显效),OCR 慢补。
**触发时机**:路径 C 下一步完成、其余泳道收口后,作为 Phase E 最后一项处理。
