...
**里程碑最终验收**(2026-07-16,大歌剧院完整 2309 图实测,v27 模型):

### ✅ M-E1 工程信息模块 —— 达成
- 健康看板四端点全 200(原 500 已修);OCR backend=rapidocr;工程信息覆盖 302/2309
  图、11.4 万条目、每条可溯源;图纸预览端点 200。

### ✅ M-E-archive 档案层单一真相源 —— 达成
- 导入即建档 + 状态机(archive_status 286 ready);人审 verified 层
  (source_kind/supersedes_key + verify 端点),闭环实证(人改标高 -2.35→99.999
  贯通到建模消费端点);坐标变换持久化 705 图。

### ⚠️ M-E2 覆盖/评测 —— 部分达成,原始 E3 数字目标被 E3-0 审计否决改道

> **重要**:§5 的 E3 验收指标(slabs≥300 / walls≥1500 / curtain_wall·steel 非零 /
> 未分层≤25% / 判专业≥95%)是**审计前基于「扩图层词表」假设**写的。E3-0 审计
> (`docs/PHASE_E_E3_AUDIT.md`)用硬数据证明歌剧院**纯 PDF 无图层、矢量文字取不到、
> 识别器无欠费**,该方案对本项目无效。以下原始数字目标**未达标、已如实改道**:

| 原目标 | 实测 | 判定 |
|--------|------|------|
| slabs 16→≥300 | 21 | ❌ 未达(审计:楼层堆叠模型板本应数十块量级,≥300 为错误预期) |
| walls 697→≥1500 | 697 | ❌ 未达(外墙闭合=线段闭环,研究级,未做) |
| curtain_wall/steel 非零 | 未加类别 | ❌ 改为 C-下一步「类型标签」(不新增顶层类别) |
| 未分层 46%→≤25% | 46% | ❌ 未达(未处理) |
| pipes 误识别↓50% | 未降噪 | ❌ 未做(研究级,有误删真管线风险) |
| 判专业≥95% | 清晰三专业 ~80% | ❌ 未达(VLM 真实上限;general 兜底类拉低) |

**改道后实际交付(审计驱动,原蓝图无)**:
| 项 | 基线→实测 | 判定 |
|----|----------|------|
| columns(围护桩圆检测) | 3089→**5704(+2615 桩)** | ✅ 真实提升 |
| slabs(桩包络补板) | 16→21 | ✅ 小幅 |
| OCR 评测(标高 97% 高置信) | `docs/PHASE_E_OCR_EVAL.md` | ✅ 落档 |
| VLM 评测(判专业 ~80% 实测) | `docs/PHASE_E_VLM_EVAL.md` | ✅ 落档 |
| 健康看板零 500 | ✅ | ✅ |

### ⏳ 数据门控三项(代码/测试完成,整机显效随 OCR 回填铺开)
| 功能 | 现状 | 门控原因 |
|------|------|----------|
| A2 轴号→3D | 仍 3/13 层、标签 "X" | 档案轴号 item 仅覆盖 302/2309 图,未铺到贡献识别轴网的结构图 |
| C 类型标签 | 逻辑通、烟测出幕墙标签 | 同上 + 变换质量(轴线多无标签) |
| E2-consume 建模读档案标高 | model_z_recovery 空 | v27 重建时 OCR 回填暂停、section 图未 OCR 完;回填铺满后重建即填 |

**结论**:M-E1/M-E-archive 完整达成(档案层架构主线跑通);M-E2 原始数字目标经审计
否决改道,改道后成果(圆检测 +2615 柱 / 类型标签 / 评测)已交付验证;三项数据门控
功能代码就绪,随 OCR 回填(4.8x 提速,现 302/2309)铺满逐步显现。
...
## 8.6 E3 实测转向:纯 PDF 项目的几何/OCR 反哺(V2 新增)

> E3-0 审计(`docs/PHASE_E_E3_AUDIT.md`)证明歌剧院 2309 图 100% PDF、无图层、
> 矢量文字取不到、识别器已抓完所有近方 poly(无欠费)。故 E3 从「扩图层词表」
> (DXF 方案,对本项目无效)转向以下两条,已按「先B再A」实施:

### E3-B 围护桩/圆柱圆检测(路径 B,已完成)

栅格化 + OpenCV HoughCircles(`core/model3d/circle_detector.py`)检圆补柱:
双闸(仅平面图 view_type=plan + 仅结构/通用专业)、尺寸下限 0.5m 排钢筋、去重、
优雅降级。**整机实测 columns 3089 → 5794(+2705 桩),剖面零误检**。

### 路径 C(A1/A2 已完成 + C-下一步 待做)

- **A1 坐标变换持久化**(migration 031 `drawing_transform`):抽取时算
  scale/origin/page_h 落库,让档案 pt 坐标可转米。705/2309 图有变换。
- **A2 档案轴号→3D**(`archive_axes_to_scene`):档案好标签(1/2/3/A/B)经 A1
  变换转米,升级识别路径 "X" 标签 + 补覆盖。代码/测试完成;整机显效随 OCR 回填铺开。
- **C-下一步(本次开工)档案 OCR 文字→构件分类**:把档案里带位置的文字
  (钢/幕墙/围护/立柱等)经 A1 变换转米,按位置就近关联到几何构件,补类型标签
  (curtain_wall/steel/exterior_wall)。与 A1 坐标基座共用,通用惠及所有 PDF 项目。

### E-末 OCR 回填提速(收尾最后一项)

**问题**:档案 OCR 289s/图 × 2309 ≈ 一天,拖慢 A2/C-下一步的整机显效。
**已实施(自适应降 DPI,~4.8x)**:A0/A1 图 200dpi 近万像素 → 分块 ~35 块 ≈ 289s;
渲染最长边 >6000px 按需降 dpi(等效 ~120dpi),分块减 ~3x:
- **配筋详图 289s → 60.6s(~4.8x)**,标高质量 0.999 不降(标签级文字低 dpi 仍清晰)。
- 全量回填 ~1 天 → ~5 小时(8 worker 并行更快)。已重启 worker 用上快速 OCR。
**未做(视需要)**:分块并行、paddle GPU、快速几何/慢速 OCR 两级拆分(A1 已单独跑过
快速变换回填验证过两级可行)。当前 4.8x 已让回填在可接受时间内铺满。
