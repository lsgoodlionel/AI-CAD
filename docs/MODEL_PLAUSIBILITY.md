...
## 六·补 · 把结论叠回图纸：反算参数（2026-09-16）

合理性引擎说「这根柱截面 0.932×0.063 m」，下一句必然是「哪根？图上是什么？」。
此前**答不了**：构件坐标用的是识别器自己算的 `scale`/`origin_pt`/`page_h`
（不是 `drawing_transform` —— 那张表构件坐标压根不读），而这三个值用完即弃。
要叠回图纸只能重跑识别，而识别代码一改（09-15 接入图框印刷比例就是一次），
存量场景就再也对不准 —— 实测 v85 的自交柱反算到页面点 **x = −170**，整个落在纸外。

现在每张图的反算参数随场景一起存：
`scene.buildings[].floors[].element_frames[drawing_id]` =
`{scale_m_pt, origin_pt, page_h, shift_m, placed}`。

**为什么必须连位移一起存**：构件并入楼层前会被共识平移、成对配准、
或世界坐标摆放动过。只用 scale/origin 反算，得到的是「识别当时」的位置，
不是场景里的位置。

**两道「不挑就丢」的关口**（代码里各有注释钉住）：
`meta` → `floor` 一道（`services/model_builder.py`），
`floor` → 单体楼层一道（`services/model_elements.py` 的单体装配）。
这两处的字段都是**显式挑选**的，算了不挑等于没算。

用法 `core/model3d/element_frame.py`：`frame_of(floor, drawing_id).to_page(x, y)`。
**逆推不了时抛异常而不是给个差不多的坐标** —— 叠错位的框看起来和叠对了一样，
只是指向别的东西，那比没有更坏。世界坐标摆放过的图（`placed=True`）是旋转+平移，
位移逆推不回去，本模块直接拒绝。

**精度边界**：柱截面模数化对齐发生在这之后，所以反算精度到模数容差为止。
实测叠图核验：南区地下一层结构平面图（一）134 根柱，视窗内 31 根**逐个压在
图上的灰色柱块上**、位置在轴线交点。同一张图上还能看见**没有红框的灰块** ——
那是漏检，这一层按设计抓不到。
