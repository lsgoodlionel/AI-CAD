...
> 只合并被整段包住的那根），v86 上估算梁 5555→1557、墙 12108→8782，
> **须重建才生效**。跨图重复（多为两张都未配准的图）有意不删，
> 理由见 `docs/MODEL_EVAL_SGOH.md` §6.1。
...
| V2.4 | 2026-09-18 | §6.11 **更正梁冗余归因**：不是总图与分图，3037/4427 为同图重复识别（梁墙从未进识别器去重判据）；新增 `dedupe.merge_collinear`（只合并被整段包住的，不用 IoU），v86 估算梁 5555→1557、墙 12108→8782，须重建生效；跨图重复有意不删 |
