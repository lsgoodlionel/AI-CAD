...
### 6.4 密排阵列剔除(座椅/吸声板/铺装单元不再算作柱)

**问题**:柱的尺寸窗口是 0.2~1.5m,而剧院座椅约 0.36~0.47m——**落在窗口正中间**,
任何纯尺寸判据都分不开。实测 60 格独立判读里 **28 格判为座椅**
(`data/model3d/gold/rule_vs_model_v1.json`)。

**判据**(`core/model3d/dense_array_filter.py`):等间距成排 **且** 间距≈自身尺寸。
柱网也等间距成排,分开两者的是**间距与自身尺寸的比**(量纲无关,不随图纸比例漂移):
座椅比值 0.94~1.00,真柱网 6.39~6.78。两条同时成立才剔除。

**剔除量**(随机样本 1209 / 4109 张 = 29%,识别器改动前的候选离线套判据。
**扫描仍在推进**;删除量**高度集中**在少数图上——388 张有候选的图里只有 37 张被命中,
样本从 288 长到 1209 张的过程中合计占比在 5.5%↔10.4% 间跳,一张 542/583 的观众席图
就能拉动整体。**该数应读作「约一成、且集中在少数图上」,不是精确估计**):
...
**误伤核验**(渲染被剔除的框逐张看):结构专业剔除的**不是真柱**——
二层结构平面图删的是柱配筋大样里的钢筋阵列、结构详图删的是**图框标题栏的汉字**、
环境总平图删的是大块填充被切出的等间距横条。阳性侧准确:观众厅整片座席
542/583 全部命中,弧形排列的「楼座137座」也整排命中。

**可见性**:被剔除的框留在 `FloorElements.dense_arrays`,**不进 `as_dict()`**
(不会流入 scene/算量/训练标签),日志记 `[model3d] 密排阵列剔除`。

**已知边界**:
| 边界 | 说明 |
|---|---|
| `DEFAULT_RUN_MIN=5` 来自实测形态 | 座椅「5 个一组 + 走道」;单张图召回对它敏感(某图 run=6 时 80%→23%),全库总量不敏感(10.2%→6.8%) |
| 顺带删掉的不止座椅 | 吸声板/铺装单元/标题栏文字/填充横条/**办公桌工位阵列**同样是密排阵列——它们本来也不是柱,但**判据抓的是几何模式,不区分语义**(模块因此叫 `dense_array_filter` 而非 `seat_filter`) |
| **斜向阵列只命中一部分** | 判据沿 x/y 轴找排,斜排时同排偏移超出容差、链断在中途——**召回不全,非误伤**。弧形排列反而命中(弧的局部近似水平) |
| 无观众席的项目也删得差不多 | 轨道交通项目 删/候选==(歌剧院 =),删的是办公家具/目录页表格/防火分区填充网格,真柱全部保留 |
| 暴露出的既有缺陷未修 | 识别器在**剖面图**和**图框标题栏**上大量产出柱候选(基坑地质剖面 338 个、标题栏汉字被当柱),这与本判据无关,是另一条待修的线 |
| 存量 scene 需重建才干净 | 重建前歌剧院 662 / 第二工程 420 根密排阵列,重建后 **0 / 5**(已落库 v85 / v8)。**注意口径**:`export_yolo_dataset.py` 里 scene 只用于**选图**,标注框来自实时 `recognize()`——识别器修好对训练标签**直接生效**,不必先重建 scene(此前误以为标注取自 scene,见 `PROGRESS.md` 2026-09-25 第十一节) |
| 场景级影响≠识别器层面 | 歌剧院 A/B(同一份代码只关座椅闸):柱 6615 → 5657(**−958 / −14.5%**),walls/beams/slabs/equipment **一字不差**。识别器层面全库 8.1%,场景级 14.5%——**没有衰减反而放大**,与 `d7002dc` 测到的「衰减 70 倍」相反,因为座椅集中在建模真正选用的观众厅平面图上 |

---

### 6.5 兜底板不再挑中图框(2026-09-10)

**问题**:`_find_slabs` 无图层命中时走兜底,按面积降序取、且「被已选中套住就跳过」
(本意防同一块楼板数两遍)。**图框恰好面积最大又套住图上一切**,于是它第一个入选、
把其余候选全部跳过 —— 每图只剩它自己。最极端一张 `S-2-32-004C`
挑中 **581,023 m²** 当楼板(一整层实际 2000~8000 m²)。

**这是 `9c0ef98` 按类配额修复暴露出来的**:配额放开前线把额度吃光、多边形只收得到
248 个,图框往往进不来。09-24 把非构件闸扩到 6 处时逐类量过 `slabs 320 → 320`
(变化 0),那批的判断在当时是对的 —— **是输入变了,不是判据坏了**。

**判据**(未新造):既有的 `is_non_component_layer()`,把 `poly_layers` 传进
`pick_fallback_slab_polygons` 即可。60 张平面图、39 张走兜底,
**39/39 的最大多边形都在图框图层上**(`C-SHET-TTLB` 35 / `图框` 3 / xref 变体 1),
形态一致:4 点轴对齐矩形、占图幅 0.9~1.0,次大是同层 48 点内框(图框是双线框)。
排除**发生在包含判定之前** —— 被排除的框若仍参与「谁套住谁」,改了等于没改。

**实测 A/B**(同一次几何抽取、同一进程,只切「传不传图层」):

| | 改前 | 改后 |
|---|---:|---:|
| 兜底板块数 | 52 | **200** |
| 折算混凝土量(板厚 0.12m,抽样内逐图相加,**非工程量**) | 519,684 m³ | **58,653 m³** |

**板数不降反增**,正是「板 18 → 6」的反向:图框不再套住候选。
唯一减少的 `ZPE002`(6 → 2)也对 —— 它改前那 6 块全是图框的格子。

**已知边界**:

| 边界 | 说明 |
|---|---|
| **这不是让板变准** | 金标准 `largest_polygon` 精确率 **3/25 = 12%**(误检:房间轮廓 32% / 墙或梁 22% / 整层外轮廓 18%)。排除图框后顶上来的仍是这些 —— `S-2-32-004C` 逐张核验:保留的 12 块里 10 块是 `S-WALL-HATCH` 墙填充、1 块是说明文字、1 块是墙线,**没有一块是真楼板**。修的是「把纸当楼」,不是「把楼认对」 |
| 部分图因此**没有板** | 按既有降级链掉到轴网包络 / 柱包络。后者 `src` 是合成标记,「图上无处可看、判读不了」——从「错得离谱」变成「无从验起」,不是变对 |
| `basis` 含义不变 | 仍标 `SLAB_BASIS_LARGEST_POLYGON`,它本就声明「不是识别结果」;`slabs_recognised` 照旧把它与真板分开数 |
| 板的**图层命中路径**未动 | 小样本里 `classify=slab 且 non_component=True` 为 **0**,与 09-24 那批一致。弱证据(只看每图前 10 个多边形),但无反证前不扩大范围 |
| 没有新设面积上限 | 图层判据已把 39/39 分干净;而 `A-FLOR-OTLN`(整层外轮廓,70,064 m²、占图幅 0.30)这类「不是图框也不是板」的,面积上限同样拦不住 |
| **存量 scene 需重建才生效** | 同其余各项:代码修好不等于到达用户 |

**探针**(可复跑):`scripts/model3d/probe_fallback_slab.py`(普查 + A/B)、
`probe_slab_frame_render.py`(红框=排除/绿框=保留,逐张画图核验)。

### 6.6 模型与图纸逐张对比查出的四处(2026-09-15)

来源:把歌剧院「1层」的构件叠回图纸原件逐张对比(当前代码 / 已落库 scene 各一张),
再用「两两偏移投票」确认两者只差一个平移 —— 单张识别基本一致,错在**比例**与**拼接**。

| # | 问题 | 判据 / 改法 | 边界 |
|---|---|---|---|
| 1 | **图框上印的比例没用上**。矢量文字轮廓化读不到 `1:N`;落库变换 1:10/1:15 被门禁挡掉;于是 `.03C/.05C` 落到缺省 1:100,构件缩小 1.5 倍 | `services/printed_scale.py` 从档案 OCR 读图框主比例(按本图 OCR 词条范围归一化位置 —— 08-31 前的档案 bbox 不是页面点;图框带票权 5;平票不猜;人审核定行直接采用)。`resolve_scale` 优先级:**矢量明文 > 图框印刷(与可信落库值不冲突时)> 可信落库 > 猜测**;桩的圆检测走同一比例 | 全库以可信落库值为准:印刷值一致 **92%**(歌剧院)/ **44%**(第二工程),缺省 1:100 为 7% / 44%。**与可信落库值冲突时让位**(轴线、档案标签、工程坐标定位都按落库变换换算,换比例会让同一张图的构件与轴线错开);冲突的 91 + 38 张不在这里裁决。**已知反例**:总图 `.01C` 图框也写 1:150,柱中位边长只有分图的 0.6 倍,真值约 1:250 |
| 2 | **板配筋、板洞当成板**。`S-0-20-102.04C` 的 139 块「板」全部来自 `0S-SLAB-RBAR`(52)与 `0S-SLAB-HOLE(-2)`(87):图层名含 SLAB,而 `_find_slabs` 的**图层命中路径**从不过非构件闸 | 非构件闸 annotation 组补 AIA 钢筋码 `RBAR`;新增 `opening` 组(`HOLE`/`OPNG`,分隔符 `-` `_` `—`);图层命中路径接闸 | 修后四张结构图各 11~12 块兜底板 —— 如实标 `largest_polygon`,**仍不是识别出的板**(§6.5 的边界照旧)。中文「板洞/洞口」图层名未量过,未加 |
| 3 | **总图与分图同进一层**。「一层结构平面总图」与分图(二)(三)(四)画同一个南区,柱被算几遍、按各自原点摆成几片;模型里分图之间重叠区的柱也对不上(0~4 根命中) | `services/sheet_series.py`:同层同系列(图名末尾「总图」↔「(一)/(2)」序号,**不认图号**)有分图时丢总图;有工程坐标定位、或分图张数超过选图配额时照留。丢了哪张记在 `floor.superseded_overviews` | **分图之间的配准没修**。试过按柱图案把分图配准到总图:各比例命中率 6~13%、没有峰 —— 柱候选精确率只有 12%,噪声对不上图案。这是 Phase J 世界坐标主线的问题 |
| 4 | **建模能力只看输入**。有 1 张剖面就标「标高·图纸实测」(实际 12 层里 11 层估算);有 1 张坐标基准图就标「世界坐标·图纸实测」 | `partial_set.reconcile_capability` 按楼层结果只往下校:有估算层 → `partial`,全部估算 → `none`;有构件的楼层部分/全无工程坐标定位 → `partial`/`none`,降级说明写明「x/N」 | 只往下校,不往上抬 |

**重建实测(歌剧院 v85 → v86,耗时 73 分钟,此前约 9 分钟)**:四处改动都按设计生效
(1506 张图用上图框比例;基础层 / 地下一层 / 1层 各有一张总图被分图替代;能力面板降为
`partial` 并写明「11/12 层标高是估算值」「只有 6/12 个有构件的楼层有工程坐标定位」),
**但整体模型变差**:柱 5657→19574、设备 1673→12931、管线 11471→33940,「1层」柱包络
220×178→400×461 米。增量的大头**不是本节四处改动**,而是 09-09/09-10 已提交、此前从未
进过 scene 的改动:

| 来源 | v85 → v86 | 说明 |
|---|---|---|
| 建筑专业的柱 | 995 → **10651** | **不只专项图**:「隔声隔振平面图」类 18 张柱约 270→5000(隔振条沿墙画成方框被当成柱,单张 `A-11-05.3B` 41→1145);**普通「平面图」19 张柱也 462→3986(8.6 倍)** |
| 机电管线 | 11471 → 33940 | 只有 **6 张图**顶到单图上限 1000,占管线 18% —— **顶格不是主因**,主因是每张图收进来的原料变多 |
| 机电设备 | 1673 → 12931 | **没有一张图顶格**(上限 300);同上,原料变多 |

> 更正(2026-09-15 同日):上一版此表把管线与设备的增长归因于「单图顶到上限」,
> 逐图统计后不成立 —— 顶格只解释 18% 的管线、0% 的设备。这些新增候选是真是假,
> 要看金标准判读(col3 批次生成于 09-11,已含配额改动,且分了建筑专业层)。
| 图框比例(本节 #1)| 「1层」约 +25% 柱 | 建筑平面由猜的 1:100 变为印刷 1:150,尺寸窗口换了一批候选;对错待金标准 |

「1层」范围仍大的原因也不在本节:按工程坐标定位的 `A-10-04.1C` 散布在 400 米范围,
分图 `.05C` 仍与其余图不相交 —— 配准问题。v85 与 v86 分别备份在
`project_models_bak_20260915` 与 `project_models_v86_20260915`,**第二工程暂不重建**。

**没改的**:金标准批次生成、YOLO 导出等脚本调 `recognize()` 时不带 `printed_scale`,
仍走旧比例路径 —— 六批判读正在进行,改了就与已发批次不可比。
...
**柱 12% → 4.6% 的来源**(09-10 图元配额按类分配):用 col3 判过的 48 格做配额前后对照
(同一批图、只换抽取规则,44 格能对上具体候选)。这批图上柱候选 **1899 → 5597(约 3 倍)**;
能对上的 42 格误检里 **30 格只在新配额下出现**、12 格旧规则下也有;2 格真柱 1 格两边都有、
1 格只在新配额下出现(旧规则下那张图只收到 271 个多边形,柱为 0)。
**两种规则的精确率都在个位数**(真柱样本只有 2 格,不能再细分)—— 配额放开主要是把候选量
放大了 3 倍;收回配额救不了精度,还会回到「按 PDF 绘制顺序截断」、随机漏掉真柱。
问题在柱的判据本身:文字、填充、标高符号、座椅都过得了尺寸窗口。

**孤立度闸(`core/model3d/isolation_filter.py`,尚未接进识别器)**:孤立度 = 最近一个别的
对象的中心距 ÷ 自身较大边长(同一根柱的外框与填充不算邻居)。col3 判过的 44 格实测:
...
真柱两格孤立度 13.5、7.2;误检在 2.1~4.4 之间很稀;`dense_array_filter` 实测真柱网
孤立度 10 分位 3.83 —— 取 k=3。按这批图全部 5597 个柱候选,k=3 删掉 **60%**。
样本内精确率约 4.5% → 8%:**闸只解决一部分**,剩下的误检是孤立的(标高符号、空白、
尺寸、文字),要靠别的判据。**真柱样本只有 2 格,量不出误删率** —— 验证批 col4
(`gold_batch --gate-isolation 3.0`)专门抽「闸删掉的」一组给判读者,误删率可接受才接线。

---

### 6.8 金标准的仪器本身:正对照(2026-09-16)

判读批次此前只有三道检查:编号校验位、**空白对照**、批内重测对。
它们能抓住「把空白说成构件」「乱抄编号」「同一格前后不一致」,
**抓不住「一律答不是」** —— 少判没有对照。实测 col4 三项全过却与 col3
差九倍,两批共同抽到的 8 张图上 col3 判出 1/13、col4 判出 12/21,
逐格核图后确认是 col3 少判。

**现在每批要混入正对照**(`core/model3d/gold/validity.positive_control_ok`):

| 项 | 规定 |
|---|---|
| 格数下限 | 6 格(少于此证明不了任何事) |
| 允许失手 | 1 格(给边界模糊留余地,不是放水) |
| 不通过 | **拒绝落库**,与空白对照同等 |
| 对照格来源 | 双人核验:判读者判真 + 维护者逐格看图复核 |
| 登记位置 | `apps/api/data/model3d/gold/positives/<kind>.tsv` |
| 存什么 | 图纸 id + 标记框 + 裁框(页面点)+ 读数 —— **不存裁图**(客户图纸不进仓库),出批次时原样重渲染 |

没有登记表的类,生成器会打印警告、回收端打印「本批没有(只体检了多判一个方向)」,
**不追认为通过**。柱已有 11 格(取自 col4),标高 7 格;轴线与锚点暂缺。

### 6.9 三类「系统读数」金标准(2026-09-16)

问的不是「这是不是构件」,而是**「系统在这里读出的东西,图上真有吗、是不是这个数」**。
候选不来自 `recognize()`,来自库里已存的抽取结果;格子右上角画出**系统读数**供对照。

| 类 | 读数来源 | 全库量 | 标记 |
|---|---|---|---|
| `elevations` | `drawing_extracted_info.category='elevation'` | 17779 条 / 2869 张图 | 红框 |
| `axis_lines` | `axis_recognition.axes` | 52718 条 / 4107 张图 | 红**虚**线(实线会盖住底下的点划线,而线型正是判据) |
| `world_anchors` | `axis_recognition.anchors` | **35 条 / 17 张图** | 红十字 |

出批次:`python -m scripts.model3d.gold_batch --kind elevations --batch elev1 ...`;
总体很小的类用 `--per-drawing` 放开每图上限(锚点用 6)。

**层高不单设类**:它是相邻标高之差,正确性由「标高读数对不对」与「楼层归属对不对」
(已有金标准 76%)决定;单造一类要么只能在全库 9 张可用剖面上做,要么是换个说法重报同一件事。

**两处坐标系陷阱(踩过并已焊进代码与测试)**:
①档案 OCR 的 bbox 量在「最长边 2160」的旧画布上,要乘 `max(page_w,page_h)/2160`;
②锚点的 `x_norm`/`y_norm` **同除页高**(`services/axis_world_anchors.py:78`),
按页宽还原会把 35 个锚点里 16 个甩到页外 —— 整格看不见标记,判读者只会答「空白」,
而**错的是还原,不是系统读数**。

### 6.10 反算参数：把模型叠回图纸核对（2026-09-16）

**问题**：合理性引擎报「这根柱截面 0.932×0.063 m」，下一句必然是「哪根？
图上是什么？」。此前答不了 —— 构件坐标用的是识别器**自己算的**
`scale`/`origin_pt`/`page_h`（**不是** `drawing_transform`，那张表构件坐标压根
不读），而这三个值用完即弃。只能重跑识别，而识别代码一改（2026-09-15 接入
图框印刷比例就是一次），存量场景再也对不准 —— 实测 v85 的自交柱反算到页面点
**x = −170**，整个落在纸外。

**现在**：`scene.buildings[].floors[].element_frames[drawing_id]` =
`{scale_m_pt, origin_pt, page_h, shift_m, placed}`，随场景一起存。

| 字段 | 含义 |
|---|---|
| `scale_m_pt` / `origin_pt` / `page_h` | 识别器当时用的换算参数 |
| `shift_m` | 并入楼层时施加的位移（共识平移 + 成对配准的累计） |
| `placed` | 是否按世界坐标摆放过（旋转+平移，**位移逆推不回去**） |

用法：`core/model3d/element_frame.py` → `frame_of(floor, drawing_id).to_page(x, y)`。
**逆推不了时抛 `FrameNotReversible`，不给「差不多」的坐标** —— 叠错位的框
看起来和叠对了一样，只是指向别的东西，比没有更坏。

**两道「不挑就丢」的关口**（各留了注释）：`meta`→`floor`（`model_builder`）、
`floor`→单体楼层（`model_elements` 单体装配）。这两处字段都是显式挑选的。

**精度边界**：柱截面模数化对齐发生在这之后，反算精度到模数容差为止。
**存量场景没有这个字段** —— 只有 2026-09-16 之后重建的模型才有。

### 6.11 v86 重建的两面性与算量口径（2026-09-16）

按当前代码重建歌剧院为 v86 并上线，逐项 A/B 见 `docs/MODEL_EVAL_SGOH.md` §6。

**修好**：`impossible` 2295 → **46**（轮廓自交 −98%，正是让 31% 的柱
混凝土量算成零的那个缺陷）；反算参数覆盖 0 → 23/23 层。

**变坏**：柱 ×3.4、墙 ×1.9、梁 ×2.1、管线 ×3.0、设备 ×7.7；
冗余构件 4422 → **11871**（梁 **80%** 冗余）。

**混凝土 34,894 → 101,636 m³。两个数都没被验证过**，但毛病说得清：
v85 ≈ 0.23 m³/m² 低于经验区间（**错在少算**，31% 柱面积为零）；
v86 在区间上沿之外且墙占 63%（**错在重复**）。

> **算量口径（管理员须知）**：**101,636 m³ 暂不可进三审**。
> 依据不是「看着偏高」，而是冗余构件的具体计数。
> 处置方向二选一：把 `geom.duplicate_element` 判出的冗余排出 QTO 总量
> （照兜底板的先例，排除但如实列出），或先治梁的 80% 冗余。
>
> **2026-09-18 更正归因**：梁冗余**不是**总图与分图 —— 4427 根里 3037 根
> 与同一张图里的另一根重合（同一条中心线叠 4/8/16 份），根因是识别器的去重
> 只覆盖柱和设备，梁墙只有中心线、从未进判据。已修（`dedupe.merge_collinear`，
> 只合并被整段包住的那根）。**已重建上线 v87**（v86 备份
> `project_models_bak_20260918`）：梁 5555→1557、墙 12108→8797，
> 同图冗余清零，冗余合计 11871→4669，混凝土 101,636→**82,260 m³**。
> **82,260 m³ 仍不可进三审** —— 剩余 4647 条跨图冗余（多为两张都未配准的图）
> 有意不删，理由见 `docs/MODEL_EVAL_SGOH.md` §6.1。
...
| V2.4 | 2026-09-18 | §6.11 **更正梁冗余归因**：不是总图与分图，3037/4427 为同图重复识别（梁墙从未进识别器去重判据）；新增 `dedupe.merge_collinear`（只合并被整段包住的，不用 IoU），已重建上线 v87：梁 5555→1557、墙 12108→8797、冗余 11871→4669、混凝土 101,636→82,260 m³（仍不可进三审）；跨图重复有意不删 |
| V2.3 | 2026-09-17 | §6.10 **反算参数**（`element_frames`）：场景存下每图的 scale/origin/page_h + 已施加位移 + 是否世界坐标摆放，模型从此能叠回图纸核对（实测南区地下一层 31 根柱逐个压在图上灰块、位置在轴线交点）；逆推不了时抛异常不给近似值；两道「不挑就丢」的关口各留注释。§6.11 **v86 重建 A/B 与算量口径**：`impossible` 2295→46（轮廓自交 −98%，即让 31% 柱方量算成零的缺陷），代价是柱 ×3.4、冗余构件 4422→11871（梁 80% 冗余），混凝土 34,894→101,636 m³ —— **暂不可进三审**，依据是冗余计数而非观感 |
| V2.2 | 2026-09-16 | §6.8 金标准补**正对照**(空白对照只抓多判;col4 三项全过却与 col3 差九倍,共同抽到的 8 张图上 1/13 vs 12/21,确认 col3 系统性少判 —— 柱 4.6% 转为存疑)。§6.9 新增三类「系统读数」金标准(标高/轴线/坐标锚点),含两处坐标系陷阱(档案 bbox 旧画布 2160、锚点同除页高)。**柱孤立度闸经 col4 验证后决定不接线**:闸保留 52% vs 闸删组 29% 判为真柱,按全库 60% 候选落在 iso<3 推算会删掉约 45% 的真柱(密排柱列),只换来精确率 ×1.37 |
