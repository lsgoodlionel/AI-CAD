...
>
> 这一路用「独立判读 + 事先声明的有效性检查」把关键数字第一次量了出来。
> 判据固定在 `data/model3d/gold/CRITERIA.md`（**每份批次命令必须从那里照抄**，
> 不照抄的后果出现过两次：柱 0.59 vs 0.22、68% vs 22%，都是判据不同导致不可比）。
>
> | 对象 | 实测 | 说明 |
> |---|---|---|
> | 墙 | **57%**（复测）/ 70%（首测）| 差值落在判读者 74% 重测信度内，视为同一水平；最大误检是**楼梯** 31% |
> | 柱 | **12%（定案）** | 固定判据 + 当前代码 + 干净对照；旧值 0.59/0.22/68%/22% 判据不同不可比。**保留组 62% 是图面标注**，标高符号占 27% |
> | 梁 | **62%**（复测）/ 0.56（首测）| 同一水平；**有「满分置信」变换记录的图梁反而更差 44% vs 78%** —— 与比例批独立同向 |
...
> | 专业 | — | 两次测量均失败（分辨率/判据），只保住「**26% 的图没有专业**」|
>
> **2026-09-15 六类批次（量的是 09-11 代码，已含图元配额按类分配，按语料加权）**：
> 柱 ~~4.6%~~ **存疑（少判，见下）** · 墙 57.5% · 梁 66.5% · 板 3.9%（整层外轮廓 21/48）·
> 管线 0% · 设备 0%。六批仪器检查全过（编号 262/262、空白对照各 8/8、重测全一致），
> 详见 `docs/GOLD_STANDARD_REVIEW.md`「判读结果（2026-09-15）」。
>
> **2026-09-16 坐标锚点（anch1）：整批被仪器拦下。** 被测 18/18 全判为对，
> 但**空白对照只有 3 格、低于事先声明的下限 6**，`gold_ingest` 拒绝落库 ——
> 那个 100% 不是结论，是没通过体检的读数。两件事叠在一起使它特别可疑：
> 空白对照在这一类上太容易过（红十字打在白纸上谁都能否掉，证明不了判读者
> 在看**位置**），而判据本身允许「看不到轴号时只判位置」，
> 所以 18/18 最多支持「十字在某两条线的交点上」，支持不了「在它声称的那个交点上」。
> 因此补了**第三种对照：偏移对照**（`gold_batch --foil`，`validity.foil_control_ok`）——
> 把真标记挪开裁框边长的 35%，形状与读数不变、裁框跟着重算，外观与被测格无异；
> 答「对」即说明没在看位置。下限 6 格、允许失手 1 格
> （锚点偏移约 6 米而柱网 8~9 米，偶尔会撞上隔壁交点，这一格的余地留给它）。
> 顺带放开了空白对照的取样来源（此前只从「有读数的图」里找，锚点类全库仅 18 张，
> 凑不够 6 格 —— 那个限制与被测对象无关）。
>
> **重出的 `anch2` 全项通过**：编号 38/38、空白 10/10、**偏移 8/8**、重测一致，
> 被测 **18/18**（语料加权 1.0，覆盖 92%）。偏移对照在它最弱的情况下也成立 ——
> 8 格里 2 格被答成 `wrong_intersection`：「附近有轴网线交会，但不是右上角标注的
> 2xB / 45xB 交点」，正是「挪 6 米撞上隔壁交点」的预期失效，判读者靠**轴号**识破，
> 这同时证明轴号可见时他在核对身份。
> **怎么读这个 100%**：n=18 不是抽样而是**全库可用总体的全部**（三倍规则下
> 95% 置信下界约 83%）；验到的是**位置**，身份只在轴号可见的格上验到。
> **它改写了 Phase J 的问题陈述**：此前把 `placed_drawings=0` 当成「配准做不准」，
> 实测是**锚点准但太少** —— 4109 张图里只有 17 张产出锚点，而这些锚点 18/18 打对。
> 「世界坐标规模化」要解的是**产量**（坐标标注识别的召回），不是配准精度。
>
> **2026-09-16 轴线定位（axis1）**：库级第一个数。仪器三项过（编号 69/69、
> 空白对照 6/7、重测 1.0），**无正对照**（只体检了多判一个方向）。
> 样本内 53.6% · **语料加权 46.2%** · 只算有把握的 43 格则 70%。
> 加权低于样本内是因为「歌剧院·mep」一层占 53% 权重而只判对 1/8。
> **最大软处**：26 条否定里 13 条判读者没把握，且**无一条判成轴线** ——
> 判不准就答「不是」，精确率系统性偏低，而这批没有正对照排除不了。
> 可引用的是区间 **46%~70%**，不是一个点。
> 与 Phase I「轴线位置 95%（103/108，**三张图**）」并不矛盾，两者一起读出的是
> **泛化差距**；那个 95% 不能当全库指标。详见 `GOLD_STANDARD_REVIEW` 第八节。
>
> **2026-09-16 标高读数（elev1，首批两个方向都验到）**：仪器四项全过
> （编号 76/76、空白对照 7/7、**正对照 6/7**、重测 1.0）。样本内 **53.6%**、
> 语料加权 86.2% —— **加权值不可单独引用**，71% 的权重压在一个只有 8 格的层上。
> 误检构成推翻了我自己的预判：26 条里 **20 条（77%）是「红框没压在标高上」**，
> 「数值不符」只有 6 条（其中 5 条确是**正负号丢失**）——**位置错才是主因**。
> 且第二工程两个层 0/8+0/8 受**红框偏移**混淆（偏移随 x 线性增长约 0.5%，
> 在档案坐标里还是在我的画布假设里**尚未定论**），那两层这一轮不能当读数精确率用。
> 详见 `docs/GOLD_STANDARD_REVIEW.md` 第七节。
>
> **2026-09-16：仪器漏掉一个方向 —— 少判没有对照。** col4（柱闸验证批）编号
> 109/109、空白对照 8/8、重测 5 对全一致、健全性零告警，**全部通过**，却与 col3
> 差九倍（被测组 52% vs 4.2%）。两批都抽到的 8 张图上：**col3 判出 1/13，
> col4 判出 12/21** —— 同样的图、同样的代码、同样的判据。逐格看图核对后确认
> **col3 是系统性少判**（把轴线交点上带剖面填充的近方块判成「细线区域·不是」），
> 它的 4.6% 不能当精确率引用。根因：**空白对照只抓「把空白说成构件」**，
> 一律答「不是」的判读者能满分通过。已补**正对照**
> （`gold/validity.positive_control_ok` + `data/model3d/gold/positives/*.tsv`，
> 双人核验、只存页面坐标不存裁图、不通过即拒绝落库）。
>
> **柱孤立度闸：不接线。** 只在 col4 内部比（唯一内部有效的对照）：闸保留 25/48（52%）、
> 闸删组 14/48 被判为真柱（误删）。全库 60% 候选落在 iso<3，按份额推算闸会
> **删掉约 45% 的真柱**，只换来精确率 ×1.37 —— 误删的正是密排柱列。
> 计算与验证批支持保留（`core/model3d/isolation_filter.py`），不接进识别器。
>
> **新增三类「系统读数」金标准（2026-09-16）**：问的不是「这是不是构件」，
> 而是「系统在这里读出的东西，图上真有吗、是不是这个数」。
> `elevations`（档案标高 17779 条）· `axis_lines`（轴线 52718 条）·
> `world_anchors`（坐标锚点**全库仅 35 条 / 17 张图**）。批次已出：elev1 / axis1 / anch1。
> 层高不单设类 —— 它是相邻标高之差，由「标高读数」×「楼层归属（76%）」两类决定。
> 建批次时查出三件事：①档案 OCR 的 bbox 在「最长边 2160」的旧画布上，
> 要乘 `max(page_w,page_h)/2160` 才是页面点；②锚点 `x_norm`/`y_norm` **同除页高**，
> 按页宽还原会把 16/35 个十字甩到页外（**量出来的就成了自己的 bug**）；
> ③标高的**负号在 OCR 那一步丢**（探针批连续出现系统读 +9.000、图上 −9.000）。
>
> **判读者本身**：重测信度 **74%** —— 差异小于这个量级的两个数不该被当成不同。
> 视觉判断可靠、**字符转写不可靠**；批次编号已改为**带校验位**的四位码。
>
> **标高符号（柱最大单一误检来源，27%）已查明进入形式并落判据**：
> 它不是以三角形、而是以 `∨` 的**一条斜笔画**进来的 —— 轴对齐包围盒
> 0.52×0.59m 近方形、落在柱窗口 (0.2~1.5m) 正中，真实尺寸却是 0.7×0.12m。
> 判据改为**最小面积外接矩形**（`core/model3d/true_extent.py`，**不新增阈值**，
...
> ③ 误检**不是随机噪声**，集中在几何签名相同的构件上（座椅→柱、楼梯→墙）；
> ④ **一个「0%」先要问它量的是什么**——上面那条否定结论量的是编码方式，
> 不是形状，把一条本可早半个月走通的路封了半个月。
>
> **未解的结构性缺口**：全库只有 **9 张**可用建筑剖面（跨视图 z 恢复的原料），
> 层高多为常量 4.5/4.2；`section_json` 与 `model_component_sections` **全空**；
> `grid_ref`（Phase H 声称的主键）只有 **42.58%** 有值。
> 存量场景比当前代码多 **46%/82%** 的柱 —— **所有修复都还没到达用户，模型需要重建**。

> **歌剧院重建 v85 → v86（2026-09-16，已上线）**：存量 v85 建于 09-02，
> 此后修掉的东西一直没到达用户。按当前代码重建后，**两面都有**，
> 逐项 A/B 见 `docs/MODEL_EVAL_SGOH.md` §6。
>
> **修好**：`impossible` 2295 → **46** —— 轮廓自交 2295→46（−98%，
> 正是**让 31% 的柱混凝土量算成零**的那个缺陷）、零面积 1687→144、
> 悬浮柱 644→21、柱长宽比异常 380→24。
> **反算参数覆盖 0 → 23/23 层（243 张图）**，模型从此能叠回图纸核对
> （实测南区地下一层结构平面图 31 根柱**逐个压在图上的灰色柱块上**、
> 位置在轴线交点；同一视窗还能看见没有红框的灰块 —— 那是漏检，这一层抓不到）。
>
> **变坏**：柱 5657→19213（×3.4）、墙 ×1.9、梁 ×2.1、管线 ×3.0、设备 ×7.7；
> 冗余构件 4422 → **11871**（**梁 80% 冗余**、墙 32%、板 38%）。
> 混凝土 34,894 → **101,636 m³**。
>
> **两个方量都没被验证，但毛病说得清**：v85 ≈ 0.23 m³/m² 低于经验区间
> （**错在少算**），v86 在区间上沿之外且墙占 63%（**错在重复**）。
> 合理性引擎判不出哪个对（它抓不到漏检），但能**逐条指出**哪 11871 个冗余 ——
> v85 时代这些问题看不见。
>
> **线上留 v86**（用户决定；2026-09-18 已由 v87 取代，见下）。**101,636 m³ 暂不可进三审** ——
> 依据是冗余的具体计数，不是观感。处置方向：把冗余排出算量（照兜底板先例），
> 或先治梁的 80% 冗余。
>
> **归因更正（2026-09-18）**：梁冗余**不是**总图与分图。4427 根里 **3037 根**
> 与**同一张图**里的另一根重合 —— 同一条中心线叠 4/8/16 份（边线在 PDF 里重画
> 几遍，配对就配出几根）。根因：识别器去重只管柱和设备、按轮廓取范围，梁墙
> 只有中心线，**从未进过判据**。已修 `core/model3d/dedupe.merge_collinear`
> （只合并被整段包住的那根，不照搬柱的 IoU>0.1 —— 那会把部分重叠的两段合成一段
> 丢长度）。**已重建上线 v87**（v86 备份 `project_models_bak_20260918`）：
> 梁 5555→**1557**、墙 12108→8797（与估算吻合），同图冗余清零，
> 冗余 11871→**4669**，混凝土 101,636→**82,260 m³**（墙 63,848→46,440、梁 3,070→1,039）。
> **82,260 m³ 仍不可进三审**：剩余 4647 条冗余全是跨图的。
> 跨图重复**有意不删**：多数是两张图都未平移，分不出「真重复」与「没配准叠在一起」，
> 见 `docs/MODEL_EVAL_SGOH.md` §6.1。
>
> **反算参数**（`scene…floors[].element_frames`，`core/model3d/element_frame.py`）：
> 每图存 `scale_m_pt/origin_pt/page_h` + 并入楼层时的位移 + 是否世界坐标摆放。
> 此前这三个值用完即弃，只能重跑识别反算，而识别代码一改就对不准 ——
> 实测 v85 的自交柱反算到页面点 **x=−170**（纸外）。逆推不了时**抛异常
> 不给近似坐标**（叠错位的框看起来和叠对了一样，只是指向别的东西）。
> 两道「不挑就丢」的关口（`meta`→`floor`、`floor`→单体楼层）各留了注释。

> **现实存在合理性分析（2026-09-16，新核心能力）**：在审图与建模两条链路里内置
> 一层**数学/物理/几何/规范下限**的规则引擎，问的不是「识别得准不准」（那要等
> 金标准人工判读），而是**「模型里这些构件在现实世界里可能存在吗」** ——
> 纯计算、零人工，每次建模后自动跑一次。代码 `core/model3d/plausibility/`
> （无 DB 依赖）、编排 `services/model_plausibility.py`、migration 052、
> 审图第 6 引擎 `core/ai_review/plausibility_engine.py`（构件由
> `core/ai_review/drawing_elements.py` 现场识别提供）、模型页「审校」模式新面板。
> 设计与边界见 `docs/MODEL_PLAUSIBILITY.md`。
>
> **三档严重度按「凭什么否定它」分**，不按「看起来多离谱」分：
> `impossible`（数学/物理不成立：轮廓自交、构件悬浮、层高≤0）·
> `implausible`（违反规范强制下限或工程量级）· `suspect`（统计离群，或所依赖
> 的限值尚未取证）。这决定了下游怎么用 —— 只有第一档能直接从算量里剔除。
>
> **四条纪律**（都对应付过的学费）：①结论必须带依据与数字，`basis` 为空直接抛异常；
> ②**跑不了 ≠ 通过** —— 缺数据抛 `RuleNotApplicable` 进 `skipped`，规则自己抛异常
> 进 `errored`，两者分开记（混为一谈会把程序 bug 说成「这模型没数据」）；
> ③**阈值必须有出处** —— `codes.py` 16 条限值里 6 条落到规范原文
> （GB55008-2021 §4.4.4 一条管住柱/梁/板/墙的最小尺寸、GB55001-2021 §4.2.2 活载表）、
> 6 条标 `EMPIRICAL`/`PHYSICS` 并写明理由、**4 条如实标 `UNVERIFIED` 保留占位不编数**
> （轴压比、长细比、跨厚比、fc 表都在 GB 50010/50017/50011，**库里没有这几本**——
> 这本身是知识库的缺口）；带 `UNVERIFIED` 的限值只能出 `suspect`；
> ④截断要说出来 —— 落库每条规则只存 50 条样例，但计数完整、被截断的列在 `truncated`。
>
> **能抓住的已知真实缺陷**：轮廓自交致混凝土量算零（存量 31.1% 的柱）、
...
| Phase C 泳道 D｜前端审校（C-16）：DrawingAnnotationQueue 深化（符号级候选框+置信度着色+确认/否定/改类/补框，标注+埋点双写，COCO 导出喂 C-09）| ✅ | `routers/model_annotations.py`、`scripts/model3d/export_annotations.py`、`tests/test_model_annotations_router.py`；**前端 `DrawingAnnotationQueue.tsx` 已于 2026-07-14 `f58159d`「模型页三模式重构」删除**，标注入口改由 `ModelWorkspace.tsx` + `useModelWorkspaceState.ts` 承接（后端端点与导出脚本原样保留） |
| Phase C 泳道 D｜前端审校（C-15）：SemanticReviewQueue 深化（拓扑闭合/命名/规范人审，低置信+规则-模型冲突优先排队，写埋点+audit_logs）| ✅ | `routers/model_review.py`、`tests/test_model_review.py`；**前端 `SemanticReviewQueue.tsx` 同在 `f58159d` 删除**，语义审校并入 `ComponentReviewQueue.tsx` 与 `ModelQualityPanel.tsx` |
...
| **密排阵列剔除（座椅不再算作柱）**：柱尺寸窗口 0.2~1.5m 覆盖座椅 0.36~0.47m，**尺寸判据不可能分开**；分开两者的是「间距÷自身边长」——座椅 0.94~1.00、真柱网 6.39~6.78。判据＝等间距成排 **且** 间距≈自身尺寸，**逐候选**（观众厅上座椅与柱共存）。误伤逐张画图核验：结构专业删的是**配筋大样里的钢筋/图框标题栏汉字/填充横条**，非真柱；阳性侧「一层楼座」542/583 与弧形「楼座137座」整排命中。被删框留 `FloorElements.dense_arrays`（不进 `as_dict()`）。**顺带查出未修**：识别器在剖面图与图框标题栏上大量产假柱候选。**场景级 A/B**（同一份代码只关闸）：柱 6615→5657（**−958 / −14.5%**），walls/beams/slabs/equipment **一字不差**；场景级 14.5% **高于**识别器层面 8.1%，**与 `d7002dc` 的「衰减 70 倍」相反**（座椅集中在建模真正选用的观众厅平面图上，所以衰减系数不可跨批换算）。scene 已重建落库（歌剧院 v85 / 第二工程 v8，密排残留 662→0 / 420→5）。**更正一处口径**：`export_yolo_dataset.py` 中 scene 只用于**选图**，标注框来自实时 `recognize()`，识别器修好对训练标签**直接生效**，不必先重建 scene | ✅ 已交付 | `core/model3d/dense_array_filter.py`、`element_recognizer.py`、`types.py`、`docs/PROGRESS.md`(2026-08-31（二）)、`MODEL_MANUAL_ADMIN.md` §6.4 |
