/**
 * 语义树候选队列（原 SemanticReviewQueue.tsx 语义节点候选部分，D-13 拆分后独立成组件）。
 *
 * 单体/分区/功能空间/施工分区候选的 confirm/reject/rename/merge/split/reparent，
 * 每个动作提交前有影响范围预览、乐观并发（version 过期提示刷新）。这套动作语义
 * （尤其 merge/split/reparent + 预览）与 UnifiedReviewInbox 的 confirm/reject/reclass/addbox
 * 不同构，因此不纳入本次收件箱合并，保留为独立面板（成果审校 topology/naming/compliance
 * 已并入 UnifiedReviewInbox，见该文件顶部注释）。
 */
import { useEffect, useMemo, useState } from 'react'
import {
  ApartmentOutlined, CheckOutlined, CloseOutlined, EditOutlined,
  EyeOutlined, ScissorOutlined, SwapOutlined,
} from '@ant-design/icons'
import {
  Alert, Button, Drawer, Empty, Form, Input, List, message,
  Modal, Select, Space, Spin, Tag, Tooltip, Typography,
} from 'antd'
import type { SemanticOperationType } from '@/services/projectModel'
import type {
  SemanticOperationDraft, SemanticOperationOutcome,
  SemanticOperationPreview, SemanticReviewItemView,
} from '../types'

const { Paragraph, Text } = Typography

interface SemanticCandidateQueueProps {
...
const SUBMIT_LABEL: Record<SemanticOperationType, string> = {
  confirm: '提交确认', reject: '提交拒绝', rename: '提交重命名',
  merge: '提交合并', split: '提交拆分', reparent: '提交父级调整',
}

const DIALOG_TITLE: Record<SemanticOperationType, string> = {
  confirm: '确认语义节点', reject: '拒绝语义节点', rename: '重命名语义节点',
  merge: '合并语义节点', split: '拆分语义节点', reparent: '调整父级',
}
...
function buildDraft(
  ...
  switch (dialogState.operation) {
    ...
    case 'split': {
      const splitNames = splitNamesText.split('\n').map((item) => item.trim()).filter(Boolean)
...
export default function SemanticCandidateQueue({
  items,
  nodeNameById,
  onPreviewOperation,
  onSubmitOperation,
  ...
  const targetOptions = useMemo(() => {
    ...
    const nodeIds = dialogState.operation === 'merge'
      ...
      : dialogState.operation === 'reparent'
        ...
        : []
    return nodeIds.map((nodeId) => ({ value: nodeId, label: nodeNameById[nodeId] ?? nodeId }))
  ...
  return (
    <>
          ...
          <List.Item key={item.nodeId}>
            <Space direction="vertical" size={10} style={{ width: '100%' }}>
              <Space align="start" style={{ width: '100%', justifyContent: 'space-between' }}>
                <Space direction="vertical" size={4} style={{ width: '100%' }}>
                  <Space wrap>
                    ...
                    {item.confidence > 0 ? <Tag color="gold">{Math.round(item.confidence * 100)}%</Tag> : null}
                  </Space>
                ...
                <Space size={4}>
                  <Tooltip title="查看证据">
                    <Button
                      type="text" icon={<EyeOutlined />} aria-label={`查看证据 ${item.title}`}
                      onClick={() => { onSelectNode?.(item.nodeId); setEvidenceItem(item) }}
                    />
                  </Tooltip>
                  <Tooltip title="确认">
                    <Button
                      type="text" icon={<CheckOutlined />} aria-label={`确认${item.title}`}
                      onClick={() => {
                        onSelectNode?.(item.nodeId)
                        setDialogState({ item, operation: 'confirm' })
                        setTargetNodeId(undefined); setNewName(item.canonicalName); setSplitNamesText('')
                      }}
                    />
                  </Tooltip>
                  <Tooltip title="调整父级">
                    <Button
                      type="text" icon={<ApartmentOutlined />} disabled={item.reparentTargets.length === 0}
                  ...
                  <Tooltip title="重命名">
                    <Button
                      type="text" icon={<EditOutlined />} aria-label={`重命名 ${item.title}`}
                      onClick={() => {
                        onSelectNode?.(item.nodeId)
                  ...
                  <Tooltip title="合并">
                    <Button
                      type="text" icon={<SwapOutlined />} disabled={item.mergeTargets.length === 0}
                  ...
                  <Tooltip title="拆分">
                    <Button
                      type="text" icon={<ScissorOutlined />} aria-label={`拆分 ${item.title}`}
                      onClick={() => {
                        onSelectNode?.(item.nodeId)
                        setDialogState({ item, operation: 'split' })
                        setTargetNodeId(undefined); setNewName(item.canonicalName)
                  ...
                  <Tooltip title="拒绝">
                    <Button
                      type="text" icon={<CloseOutlined />} aria-label={`拒绝 ${item.title}`}
                      onClick={() => {
                        onSelectNode?.(item.nodeId)
              ...
              {staleVersions[item.nodeId] ? (
                <Alert
                  type="warning" showIcon message="语义树版本已更新"
      ...
      <Drawer
        ...
        footer={<Button onClick={() => setEvidenceItem(null)}>关闭证据</Button>}
      >
            ...
            <Alert
              ...
              message={(
                <Space wrap>
                  ...
                  {typeof evidence.score === 'number' ? <Tag color="gold">{Math.round(evidence.score * 100)}%</Tag> : null}
                </Space>
              )}
              description={(
                <Space direction="vertical" size={4}>
                  <Text>{evidence.detail}</Text>
                  {evidence.sourceDrawingId ? <Text type="secondary">来源图纸 {evidence.sourceDrawingId}</Text> : null}
                </Space>
              )}
            />
          ))}
      ...
      <Modal
        ...
        onCancel={() => { setDialogState(null); setPreview(null) }}
        destroyOnClose
        footer={[
          <Button key="cancel" onClick={() => { setDialogState(null); setPreview(null) }}>取消</Button>,
          <Button
            key="submit" type="primary" loading={submitting} disabled={!currentDraft}
            onClick={async () => {
              if (!currentDraft || !dialogState) return
              setSubmitting(true)
              try {
                const result = await onSubmitOperation(currentDraft)
                if (result.ok) { setDialogState(null); setPreview(null); return }
                if (result.staleVersion) {
                  setStaleVersions((current) => ({ ...current, [dialogState.item.nodeId]: result.staleVersion! }))
                  setDialogState(null); setPreview(null)
                }
              } finally {
                setSubmitting(false)
              }
        ...
        {dialogState ? (
          <Space direction="vertical" size={14} style={{ width: '100%' }}>
            ...
            {dialogState.operation === 'rename' ? (
              <Form layout="vertical">
                <Form.Item label="新的名称" style={{ marginBottom: 0 }}>
                  <Input aria-label="新的名称" value={newName} onChange={(event) => setNewName(event.target.value)} />
                </Form.Item>
              </Form>
            ) : null}
            ...
            {(dialogState.operation === 'merge' || dialogState.operation === 'reparent') ? (
              <Form layout="vertical">
                <Form.Item label={dialogState.operation === 'merge' ? '合并目标' : '新的父级'} style={{ marginBottom: 0 }}>
                  <Select
                    aria-label={dialogState.operation === 'merge' ? '合并目标' : '新的父级'}
                    options={targetOptions} value={targetNodeId}
                    onChange={(value) => setTargetNodeId(value)} placeholder="请选择目标节点"
                  />
                </Form.Item>
              </Form>
            ) : null}
            ...
            {dialogState.operation === 'split' ? (
              <Form layout="vertical">
                <Form.Item label="拆分名称（每行一个）" style={{ marginBottom: 0 }}>
                  <Input.TextArea
                    aria-label="拆分名称" autoSize={{ minRows: 3, maxRows: 6 }}
                    value={splitNamesText} onChange={(event) => setSplitNamesText(event.target.value)}
                  />
                </Form.Item>
              </Form>
            ) : null}
            ...
            <Space direction="vertical" size={8} style={{ width: '100%' }}>
              ...
              {previewLoading ? (
                ...
                <Alert
                  type="info" showIcon message={preview.summary}
                  description={(
                    <Space direction="vertical" size={4}>
                      {preview.affected_scope.map((scope) => <Text key={scope}>{scope}</Text>)}
                      {preview.fallback_reason ? (
                        <Paragraph type="secondary" style={{ marginBottom: 0 }}>{preview.fallback_reason}</Paragraph>
                      ) : null}
                    </Space>
                  )}
                />
