...
async def main():
    ...
    for f in sc.get("floors", []):
        for w in (f.get("elements") or {}).get("beams") or []:
            p = w.get("path") or []
    ...
    random.shuffle(srcs)
    print(f"有 ≥6 条梁且比例可信的图 {len(srcs)} 张")
    ...
    for did in srcs:
        ...
        if not row:
            continue
        # **只取平面图**：详图/配筋图上的「墙」是断面大样，不是平条梁，
        # 判读者会被迫在两种语境间来回切换
        t = str(row["title"] or "")
        # **梁的主力图种正是「梁平面整体配筋图」** —— 对墙成立的「排除配筋图」
        # 这条，对梁不成立。仍排除详图/大样/剖面（那里的梁是断面）。
        if "平面" not in t or any(k in t for k in ("详图", "大样", "剖面")):
            continue
        try:
    ...
    for row, fe, page, w in picks:
        if True:
            ...
            tiles.append((img.resize((CELL, CELL), Image.LANCZOS),
                          row["id"], f"{w.get('width',0)}x{w.get('depth',0)}"))
    ...
    for si in range((len(tiles) + per - 1) // per):
        ...
        sid = f"B{si+1}"
        d0.text((12, 8), f"{sid}   {len(chunk)} 个梁候选  编号 {sid}-01 ~ {sid}-{len(chunk):02d}",
