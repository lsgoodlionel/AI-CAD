...
#:
#: 判据来自实测的「各层最大/中位」分布：
#: 观感可接受的大歌剧院 中位 **2.56**、P75 3.27；
#: 观感差的轨道交通 中位 **7.56**、P75 10.16、最大 17.12。
#: 取 4.0 —— 健康工程 P75 加余量。
#:
#: **这个门槛只是兜底**：配额加大后视图变散，根子不在门槛，
#: 而在**更多图纸意味着更多互不一致的局部坐标系**（每张图坐在自己的
#: 原点上）。真正的解法是 Phase K 的轴网帧；
#: 一味调低门槛只是把内容藏起来，覆盖面涨一次就要再藏一批。
MAX_SPAN_RATIO = 4.0
