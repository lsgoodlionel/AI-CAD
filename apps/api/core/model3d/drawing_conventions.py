...
CLAUSES: dict[str, dict] = {
    ...
    "plan_cut_sections": {
        "text": "(工程约束)水平剖切得到的构件截面(柱/墙的平面轮廓)"
                "不会出现在剖面图上——剖面是竖直剖切面上的投影。",
        "applied_in": "本模块 shows_plan_cut_sections → "
                      "core.model3d.element_recognizer._recognize",
        "evidence": "全库扫描 + 逐张叠框核验:剖面图上柱候选 338/338/198/51/54/17 根"
                    "(地质剖面三张/围护体剖面两张/装修 1-1 剖面),"
                    "无一根是柱——地层填充格、立柱桩的横缀条、竣工章、标高刻度。"
                    "**立面与详图不在此条内**:实测各有反例(见 shows_plan_cut_sections)。",
    },
    "8.0.7": {
...
def shows_plan_cut_sections(view_type: str | None) -> bool:
    """这张图上**会不会出现**水平剖切得到的构件截面(柱/墙的平面轮廓)。

    识别器靠尺寸猜柱,而尺寸判据在剖面图上分不开真假:一个 3.5mm 字高的
    汉字在 1:100 下换算 0.35 米,正落在柱的尺寸窗口(0.2~1.5m)当中;
    地层填充格、立柱桩横缀条也一样。分得开的是**图种**。

    **只对 `section` 说不。** 立面与详图看起来同理,实测各有反例,
    而反例删的正是真柱:

    - `elevation`:「北立面构造柱定位图」图名是立面,图内每一格却是
      「N 层北立面构造柱**平面**定位图 1:100」—— 57 个候选全部压在真构造柱上。
    - `detail`:`match_view_type_keyword` 把「详图/大样/节点」一律判为 detail,
      而「楼梯 ST-18、19 结构详图」「2 结构详图(一)」的每一格是
      「楼梯 ST-xx 标高 -9.300 **结构平面图**」,红框压在真梯柱(TZ)上。

    两个反例是同一个病:**图幅级的图种不等于图幅内每一格的图种**,
    而格级图名读不到(这两个工程实测每张只能取到 4 个可信词)。
    等能按格判图种了再放开这两档,不要靠图名猜。

    判不出图种(unknown/空)一律 True:**缺失不得阻断**。
    """
    return str(view_type or "").strip().lower() != "section"
