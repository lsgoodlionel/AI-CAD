...
TokenKind = Literal[
    ...
    # 标题栏**字段标签**（ISO 7200 / GB/T 10609.1）：`PROJECT` / `比例` /
    # `审核`……是表头不是值。实测两个工程各有近 3 万 / 1 万条落在 `other`，
    # 把 `other` 占比抬到 51% / 37%，掩盖了真正分类不出的内容。
    "title_block_label",
    # **平法构件编号**（22G101）：`KZ1` / `KL2(3)` / `Q1` / `LB1` / `M1124`。
    # 19 个专业 133 条会审检查项里「编号」出现 38 条 —— 人核图的主要抓手：
    # 看到平面图标 `KZ1`，就去柱表查它的配筋与截面。
    "component_mark",
    # **构造做法层**：`20厚DSM20预拌砂浆找平层`。会审 133 条检查项里
    # 「做法」出现 47 条 —— 它通往真实构造层次（建模厚度、算量材料）。
    "construction_layer",
    # **管道公称直径**（GB/T 1047）：`DN100` / `De75`。
    # 管径此前是硬编码的，而图上白纸黑字标着 —— 实测大歌剧院
    # 4047 条覆盖 303 张图。
    "pipe_diameter",
