...
#: 同步函数，被放弃的图**无论如何都会被算完**、线程照样占着池。
#: 超时只决定「要不要用这个已经算出来的结果」——
#: 同样的代价下，20 秒是丢掉，60 秒是拿到。
#:
#: 实测三张此前被跳过的柱平面图（空闲机器，2 MP 圆检测预算下）：
#:
#: | 图 | 耗时 | 柱 |
#: |---|---:|---:|
#: | S-1-32-103C | 17.7 s | 18 |
#: | S-1-32-106C | 22.0 s | 17 |
#: | S-1-32-201C | **40.6 s** | 22 |
#:
#: 取 60 秒：覆盖最慢的 40.6 秒并留 50% 余量（重建时多线程竞争 CPU，
#: 实际耗时比单独跑长 —— 103C 单跑 17.7 秒仍在重建中被判超时）。
_RECOGNIZE_TIMEOUT_SEC = 60
...
#: 序列校验(§8.0.3)兜底。
#: 构件配额加大后（structure 2→6），12 已不再「远大于」它。
#: 轴网聚合几乎不花时间，所以它应当**随构件配额一起抬**，
#: 而不是被一个固定数字锁死——图越多，共识越稳。
MAX_AXIS_SOURCE_PLANS = _quota(
    "CAD_MAX_AXIS_SOURCE_PLANS", max(24, _MAX_STRUCTURE_PLANS * 4))
