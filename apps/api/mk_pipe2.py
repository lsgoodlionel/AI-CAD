...
async def main():
    ...
    rows = await db.fetch_all(
        ...
        "WHERE d.title LIKE '%平面图%' AND d.discipline = 'mep' "
        "AND d.title NOT LIKE '%详图%' AND d.title NOT LIKE '%大样%' "
        "AND d.title NOT LIKE '%系统%' AND d.title NOT LIKE '%原理%' "
    ...
    for row in rows:
        ...
        ws = [w for w in fe.pipes if len(w.get("path") or []) == 2]
        ...
        try:
            ...
            pass
        gc.collect()
    print(f"管线候选 {len(picks)} · 空白对照 {len(blanks)}")

    allp = [(r, im, False) for r, im in picks] + [(r, im, True) for r, im in blanks]
    random.shuffle(allp)
    codes = make_codes(len(allp) + 20, seed=20260921)
    try:
        font = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 26)
    except Exception:
        font = ImageFont.load_default()
    CELL, COLS, ROWS = 400, 5, 4
    tiles = [(codes.pop(), "blank" if is_blank else "pipe", str(row["id"]),
              str(row["title"] or ""), im.resize((CELL, CELL)))
             for row, im, is_blank in allp]
