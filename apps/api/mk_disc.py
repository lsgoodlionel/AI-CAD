...
专业判别是被到处使用的整数级属性（楼层管 z，单体管 xy 分组），同样是**整模型级**
属性、同样从未量过。实测：
...
async def main():
    ...
    for nm, pid in PROJECTS.items():
        rows = await db.fetch_all(
            "SELECT id,discipline FROM drawings "
            "WHERE project_id::text=:p AND title IS NOT NULL", {"p": pid})
        pool = collections.defaultdict(list)
        for r in rows:
            pool[str(r["discipline"] or "?")].append(str(r["id"]))
        for key, dids in sorted(pool.items()):
            random.shuffle(dids)
            # 每个专业每个工程抽 10 张；`general` 是解析不出时的默认值，全抽
            for did in dids[:10]:
    ...
    for si in range(0, len(tiles), COLS * ROWS):
        ...
        p = f"{OUT}/D{si // (COLS * ROWS) + 1}.png"
        sh.save(p); sheets.append(p)
    with open(f"{OUT}/manifest.tsv", "w") as f:
        f.write("tag\tproject\tassigned_disc\tdrawing_id\ttitle\tdb_discipline\n")
        for x in man:
            f.write("\t".join(x) + "\n")
